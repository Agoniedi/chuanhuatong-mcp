...
着一条混合人/AI 的消息流协作）与手机（碎片化查看），两者都重要。当前 Web 端是
**只读的消息查看器**：用户在此阅读、跟随房间对话，但不通过 Web 直接发消息或调用
MCP 工具；发送与 Agent 发言由 MCP Host 端完成。
...
- 约束：Web 端为只读，永不直接调用 `/mcp`；浏览器 WebSocket 认证通过 `?token=` 查询
...
3. **顺序与可信为先**：按权威 `seq` 呈现，实时但可恢复（WS 至少一次 + REST 权威回补），
   连接状态对用户透明。
4. **只读的克制**：Web 端如实呈现房间真相，不伪装成可发送端，不越权触碰 MCP。
